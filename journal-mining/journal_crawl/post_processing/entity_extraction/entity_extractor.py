		...
		for i in range(len(mylist)):
			if i in rolenum:
				if i != 0: #not the first word
					all_content.append((title, current_role, current_sent))
					current_sent = ""
...
def entity_extractor(publisher):
	...
	data = database_helper.DatabaseHelper.getTaggedLines(config.DB_FILEPATH, publisher)
	...
	for title, role, content in all_content:
		...
		if perReady and orgReady: #both field are ready to input
			...
			database_helper.DatabaseHelper.addCrawledItem(items.CrawledItem(publisher=publisher, title=journal, role = role, name=name, affiliation = affiliation), config.DB_FILEPATH)
