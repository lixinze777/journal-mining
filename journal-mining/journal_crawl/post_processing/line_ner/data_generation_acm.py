...
def preTagging(roleTable):
	...
	cur = conn.cursor()
	data = cur.execute("SELECT _line FROM journalLine WHERE publisher = 'ACM' AND _line != \"\"").fetchall()
...
def generateTestCase(tagword, taggedData, TEST_THRESHOLD):
	...
	file1 = open("datatrainACM.txt", "a")
	for item in train:
		file1.writelines(item)
	file1.close()

	file2 = open("datadevACM.txt", "a")
	for item in dev:
		file2.writelines(item)
	file2.close()

	file3 = open("datatestACM.txt", "a")
	for item in test:
