...
class ACMjournal:
	...
	def __init__(self):
		...
		self.driver = webdriver.Chrome(executable_path='/home/wing.nus/xinze/journal-mining/journal_crawl/journal_crawl/spiders/driver/chromedriver', options=options)
	...
	def parse_page(self, html):
		...
		while counter < len(url_list):
			...
			database_helper.DatabaseHelper.addJournal(items.JournalInfoItem(title=title, publisher="ACM", url=url), config.DB_FILEPATH)
			counter = counter + 1
